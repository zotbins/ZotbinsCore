...
float get_fullness(void) {
    ...
    if (fullness > 100.0 && fullness < 110.0) { // Likely a sensor measurement error, cap at 100%
        fullness = 100.0;
    } 

    else if (fullness < 0.0) { // Floor at 0% --- THIS CAN CAUSE CONFUSION, MAKE SURE THE MAXIMUM DISTANCE IS CALIBRATED PROPERLY
        fullness = 0.0;
    }

    else if (fullness >= 110.0) { // Something is probably wrong, time to check up on the bin...

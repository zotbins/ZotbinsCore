...
void advanced_ota_example_task(void *pvParameter)
{
    ...
    while (1) {
        ...
        if (err != ESP_ERR_HTTPS_OTA_IN_PROGRESS) {
            ESP_LOGE(TAG, "AHHHHH ITS BROKENNNNNNNNNN");
            break;
        }
        
        // Get total binary file size from Content-Length
        int total_len = esp_https_ota_get_image_size(https_ota_handle);
        if (total_len <= 0) {
            // ESP_LOGW(TAG, "Unable to determine file size");
        } else {
            ESP_LOGI(TAG, "Total binary file size: %d bytes", total_len);
        }
        ...
        if (total_len > 0) {
            float progress = (float)current_len / total_len * 100.0f;
            ESP_LOGI(TAG, "Downloaded: %d / %d bytes (%.1f%%)", current_len, total_len, progress);
        } else {
            ESP_LOGI(TAG, "Downloaded: %d bytes / Unknown", current_len);
        }
    }
